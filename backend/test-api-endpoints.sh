...
ZK_PAYLOAD='{
  ...
  "publicSignals": ["public_signal_1", "public_signal_2"],
  "commitment": "sample_commitment_for_zk_verification"
}'
...
test_post_endpoint "$DEPLOYED_URL" "/api/auth/login" "User Login (POST)" '{"walletAddress":"test_address","signature":"test_signature"}'
test_get_endpoint "$DEPLOYED_URL" "/api/auth/login" "User Login (GET - should fail)"
test_post_endpoint "$DEPLOYED_URL" "/api/auth/verify" "Token Verification (POST)" '{"token":"test_token"}'
...
test_post_endpoint "$DEPLOYED_URL" "/api/utxo/deposit" "UTXO Deposit (POST)" "$UTXO_PAYLOAD"
test_get_endpoint "$DEPLOYED_URL" "/api/utxo/deposit" "UTXO Deposit (GET - should fail)"
test_post_endpoint "$DEPLOYED_URL" "/api/utxo/refund" "UTXO Refund (POST)" "$REFUND_PAYLOAD"
test_get_endpoint "$DEPLOYED_URL" "/api/utxo/balance" "UTXO Balance (GET)"
test_post_endpoint "$DEPLOYED_URL" "/api/utxo/history" "UTXO History (POST)" '{"walletAddress":"test_address"}'
...
test_post_endpoint "$DEPLOYED_URL" "/api/zk/verify" "ZK Proof Verification (POST)" "$ZK_PAYLOAD"
test_get_endpoint "$DEPLOYED_URL" "/api/zk/verify" "ZK Proof Verification (GET - should fail)"
test_post_endpoint "$DEPLOYED_URL" "/api/zk/commitment" "ZK Commitment (POST)" '{"commitment":"test_commitment","nullifier":"test_nullifier"}'
test_post_endpoint "$DEPLOYED_URL" "/api/zk/proof" "ZK Proof Generation (POST)" '{"secret":"test_secret","publicSignal":"test_signal"}'
...
test_post_endpoint "$DEPLOYED_URL" "/api/user/profile" "User Profile (POST)" '{"walletAddress":"test_address"}'
...
if curl -s --connect-timeout 2 "$LOCAL_URL/api/health" >/dev/null 2>&1; then
    ...
    test_post_endpoint "$LOCAL_URL" "/api/utxo/deposit" "Local UTXO Deposit (POST)" "$UTXO_PAYLOAD"
    
    echo
    echo "🔒 LOCAL ZK ENDPOINTS"
    test_post_endpoint "$LOCAL_URL" "/api/zk/verify" "Local ZK Verification (POST)" "$ZK_PAYLOAD"
else
...
echo "curl -X POST -H 'Content-Type: application/json' -d '$UTXO_PAYLOAD' $DEPLOYED_URL/api/utxo/deposit"
echo
echo "# Test ZK verification:"
echo "curl -X POST -H 'Content-Type: application/json' -d '$ZK_PAYLOAD' $DEPLOYED_URL/api/zk/verify"
echo
