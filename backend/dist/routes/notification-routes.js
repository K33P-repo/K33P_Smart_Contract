...
import { NotificationModel, NotificationPreferenceModel, UserModel } from '../database/models.js';
 ...
 */
router.get('/', authenticateToken, query('is_read').optional().isBoolean().withMessage('is_read must be a boolean'), query('is_seen').optional().isBoolean().withMessage('is_seen must be a boolean'), query('notification_type').optional().isString().withMessage('notification_type must be a string'), query('priority').optional().isString().withMessage('priority must be a string'), query('start_date').optional().isISO8601().withMessage('start_date must be a valid ISO date'), query('end_date').optional().isISO8601().withMessage('end_date must be a valid ISO date'), query('limit').optional().isInt({ min: 1, max: 100 }).withMessage('limit must be between 1 and 100'), query('offset').optional().isInt({ min: 0 }).withMessage('offset must be a non-negative integer'), query('order_by').optional().isIn(['created_at', 'priority', 'sent_at']).withMessage('order_by must be one of: created_at, priority, sent_at'), query('order_direction').optional().isIn(['asc', 'desc']).withMessage('order_direction must be asc or desc'), handleValidationErrors, handleAsyncRoute(async (req, res) => {
    try {
        const authUser = req.user; // Get the user object
        ...
        const { is_read, is_seen, notification_type, priority, start_date, end_date, limit = 20, offset = 0, order_by = 'created_at', order_direction = 'desc' } = req.query;
        const filter = {
 ...
 */
router.get('/preferences', authenticateToken, handleAsyncRoute(async (req, res) => {
...
router.get('/stats', authenticateToken, handleAsyncRoute(async (req, res) => {
    try {
        const authUser = req.user; // Get user object
        const userId = authUser.userId;
 ...
 */
router.get('/:id', authenticateToken, param('id').isUUID().withMessage('Valid notification ID is required'), handleValidationErrors, handleAsyncRoute(async (req, res) => {
    try {
        const authUser = req.user; // Get user object
        const userId = authUser.userId;
 ...
 */
router.post('/', authenticateToken, validateNotificationCreate, handleValidationErrors, handleAsyncRoute(async (req, res) => {
    try {
        const authUser = req.user; // Renamed to avoid conflict
        ...
        const isAllowed = await NotificationPreferenceModel.isNotificationAllowed(notificationData.user_id, notificationData.notification_type || 'system');
        if (!isAllowed) {
            return res.status(400).json({
                success: false,
 ...
 */
router.put('/:id/read', authenticateToken, param('id').isUUID().withMessage('Valid notification ID is required'), handleValidationErrors, handleAsyncRoute(async (req, res) => {
    try {
        const authUser = req.user; // Get user object
        const userId = authUser.userId;
        const notificationId = req.params.id;
 ...
 */
router.put('/:id/seen', authenticateToken, param('id').isUUID().withMessage('Valid notification ID is required'), handleValidationErrors, handleAsyncRoute(async (req, res) => {
    try {
        const authUser = req.user; // Get user object
        const userId = authUser.userId;
        const notificationId = req.params.id;
 ...
 */
router.put('/read-all', authenticateToken, handleAsyncRoute(async (req, res) => {
    try {
        const authUser = req.user; // Get user object
        const userId = authUser.userId;
 ...
 */
router.put('/seen-all', authenticateToken, handleAsyncRoute(async (req, res) => {
    try {
        const authUser = req.user; // Get user object
        const userId = authUser.userId;
 ...
 */
router.delete('/:id', authenticateToken, param('id').isUUID().withMessage('Valid notification ID is required'), handleValidationErrors, handleAsyncRoute(async (req, res) => {
    try {
        const authUser = req.user; // Get user object
        const userId = authUser.userId;
        const notificationId = req.params.id;
 ...
 */
router.delete('/', authenticateToken, body('notification_ids')
    .isArray()
    .withMessage('notification_ids must be an array')
    .custom((ids) => ids.every(id => /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(id)))
    .withMessage('All notification IDs must be valid UUIDs'), handleValidationErrors, handleAsyncRoute(async (req, res) => {
    try {
        const authUser = req.user; // Get user object
        const userId = authUser.userId;
 ...
 */
router.put('/preferences/:type', authenticateToken, param('type')
    .isIn(['system', 'transaction', 'security', 'subscription', 'wallet', 'backup', 'emergency', 'promotion'])
    .withMessage('Invalid notification type'), handleAsyncRoute(async (req, res) => {
    try {
        const authUser = req.user; // Get user object
        const userId = authUser.userId;
        ...
        if (req.body.email_enabled !== undefined)
            updates.email_enabled = req.body.email_enabled;
        if (req.body.push_enabled !== undefined)
            updates.push_enabled = req.body.push_enabled;
        if (req.body.sms_enabled !== undefined)
            updates.sms_enabled = req.body.sms_enabled;
        if (req.body.quiet_hours_start !== undefined)
            updates.quiet_hours_start = req.body.quiet_hours_start;
        if (req.body.quiet_hours_end !== undefined)
            updates.quiet_hours_end = req.body.quiet_hours_end;
 ...
 */
router.get('/preferences/quiet-hours', authenticateToken, handleAsyncRoute(async (req, res) => {
    try {
        const authUser = req.user; // Get user object
        const userId = authUser.userId;
...
router.post('/system/transaction', authenticateToken, body('user_id').isString().withMessage('User ID is required'), body('title').isString().withMessage('Title is required'), body('message').isString().withMessage('Message is required'), body('tx_hash').optional().isString().withMessage('Transaction hash must be a string'), handleValidationErrors, handleAsyncRoute(async (req, res) => {
    try {
 ...
 */
router.post('/system/security', authenticateToken, body('user_id').isString().withMessage('User ID is required'), body('title').isString().withMessage('Title is required'), body('message').isString().withMessage('Message is required'), handleValidationErrors, handleAsyncRoute(async (req, res) => {
    try {
        const { user_id, title, message } = req.body;
        logger.info(`Creating security notification for user: ${user_id}`);
        const notification = await NotificationModel.createSecurityNotification(user_id, title, message);
        res.status(201).json({
            success: true,
