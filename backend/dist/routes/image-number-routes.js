 ...
 */
router.get('/', authenticateToken, handleAsyncRoute(async (req, res) => {
    try {
 ...
 */
router.put('/', authenticateToken, validateImageNumber, handleValidationErrors, handleAsyncRoute(async (req, res) => {
    try {
        const userId = req.user?.userId;
 ...
 */
router.get('/preview', authenticateToken, handleAsyncRoute(async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            return res.status(401).json({
 ...
 */
router.put('/random', authenticateToken, handleAsyncRoute(async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            return res.status(401).json({
 ...
 */
router.put('/reset', authenticateToken, validateResetConfirmation, handleValidationErrors, handleAsyncRoute(async (req, res) => {
    try {
        const userId = req.user?.userId;
...
//# sourceMappingURL=image-number-routes.js.map