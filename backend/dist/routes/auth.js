...
router.post('/verify-wallet', verifyToken, async (req, res) => {
  try {
...
router.get('/wallet-connect', verifyToken, async (req, res) => {
  try {
...
const blockfrost = new BlockFrostAPI({
  projectId: process.env.BLOCKFROST_API_KEY || 'preprodbl7bIxYc2sbEeGAZyo2hpkjJwzOAQNtG',
  network: 'preprod'
});
...
router.post('/verify-wallet', verifyToken, walletVerifyLimiter, async (req, res) => {
  try {
    const { walletAddress } = req.body;
    const userId = req.user.userId;
