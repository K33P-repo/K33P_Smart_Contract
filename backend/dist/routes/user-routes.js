 ...
 */
router.post('/username/create', authenticateToken, body('userName')
    .trim()
    .isLength({ min: 2, max: 50 })
    .withMessage('Username must be between 2 and 50 characters')
    .matches(/^[a-zA-Z0-9_-]+$/)
    .withMessage('Username can only contain letters, numbers, underscores, and hyphens'), handleValidationErrors, handleAsyncRoute(async (req, res) => {
    try {
        const userId = req.user.id;
        const { userName } = req.body;
        ...
        try {
            const checkResult = await checkClient.query('SELECT user_id FROM users WHERE user_name = $1 AND user_id != $2', [userName, userId]);
        ...
        const updateClient = await pool.connect();
        try {
            await updateClient.query('UPDATE users SET user_name = $1, updated_at = CURRENT_TIMESTAMP WHERE user_id = $2', [userName, userId]);
        }
        finally {
            updateClient.release();
        }
 ...
 */
router.put('/username/edit', authenticateToken, body('userName')
    .trim()
    .isLength({ min: 2, max: 50 })
    .withMessage('Username must be between 2 and 50 characters')
    .matches(/^[a-zA-Z0-9_-]+$/)
    .withMessage('Username can only contain letters, numbers, underscores, and hyphens'), body('currentUserName')
    .optional()
    .isString()
    .withMessage('Current username must be a string'), handleValidationErrors, handleAsyncRoute(async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        let user = null;
        try {
            const result = await client.query('SELECT user_name FROM users WHERE user_id = $1', [userId]);
            if (result.rows.length > 0) {
                user = result.rows[0];
            }
        }
        ...
        try {
            ...
            const lastChangeResult = await updateClient.query('SELECT updated_at FROM users WHERE user_id = $1', [userId]);
 ...
 */
router.get('/username/check-availability', body('userName')
    .trim()
    .isLength({ min: 2, max: 50 })
    .withMessage('Username must be between 2 and 50 characters')
    .matches(/^[a-zA-Z0-9_-]+$/)
 ...
 */
router.delete('/username/remove', authenticateToken, body('confirmPin')
    .optional()
    .isLength({ min: 4, max: 6 })
    .isNumeric()
    .withMessage('PIN must be 4-6 digits'), body('confirmPassword')
    .optional()
    .isString()
    .withMessage('Password confirmation must be a string'), handleValidationErrors, handleAsyncRoute(async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        let user = null;
        try {
            const result = await client.query('SELECT user_name, pin_hash FROM users WHERE user_id = $1', [userId]);
            if (result.rows.length > 0) {
                user = result.rows[0];
            }
        }
        ...
        const updateClient = await pool.connect();
        try {
            await updateClient.query('UPDATE users SET user_name = NULL, updated_at = CURRENT_TIMESTAMP WHERE user_id = $1', [userId]);
        }
        finally {
            updateClient.release();
        }
