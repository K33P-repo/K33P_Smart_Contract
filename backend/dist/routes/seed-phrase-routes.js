...
], handleValidationErrors, auditLogger('STORE', 'SEED_PHRASE'), async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            return res.status(401).json({ success: false, message: 'User not authenticated' });
        }
        const { walletName, walletType, mnemonicType, seedPhrase, encryptionPassword } = req.body;
        const result = await seedPhraseService.storeSeedPhrase(userId, walletName, seedPhrase, walletType);
        res.status(201).json({
            success: true,
 ...
 */
router.get('/', authenticateToken, auditLogger('LIST', 'SEED_PHRASE'), async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            return res.status(401).json({ success: false, message: 'User not authenticated' });
...
], handleValidationErrors, auditLogger('RETRIEVE', 'SEED_PHRASE'), async (req, res) => {
    try {
        const userId = req.user?.userId;
        ...
        const result = await seedPhraseService.retrieveSeedPhrase(seedPhraseId, userId, encryptionPassword);
        res.json({
            success: true,
...
//# sourceMappingURL=seed-phrase-routes.js.map