...
router.post('/initialize', authenticateToken, async (req, res) => {
    try {
        ...
        try {
            const userResult = await client.query('SELECT email, phone_hash FROM users WHERE user_id = $1', [userId]);
        ...
        const subscriptionResult = await client.query('SELECT tier FROM subscriptions WHERE user_id = $1 AND tier = $2 AND is_active = true', [userId, 'premium']);
            ...
            if (result.data?.status === 'success' && result.data?.authorization) {
                ...
                try {
                    ...
                    if (userId) {
                        ...
                        await client.query(`INSERT INTO subscriptions (user_id, tier, is_active, start_date, end_date, auto_renew, authorization_code)
               ...
               DO UPDATE SET 
                 ...
                 updated_at = CURRENT_TIMESTAMP`, [userId, startDate, endDate, result.data.authorization.authorization_code]);
...
router.post('/charge-recurring', authenticateToken, async (req, res) => {
    try {
        ...
        try {
            ...
            const subscriptionResult = await client.query('SELECT authorization_code FROM subscriptions WHERE user_id = $1 AND tier = $2 AND is_active = true', [userId, 'premium']);
            ...
            if (chargeResult.success) {
                ...
                await client.query('UPDATE subscriptions SET end_date = $1, updated_at = CURRENT_TIMESTAMP WHERE user_id = $2', [newEndDate, userId]);
...
router.post('/initialize-monthly', authenticateToken, async (req, res) => {
    try {
        ...
        const result = await subscriptionService.initializeMonthlyRecurringSubscription(userId, phone, amount || 5000 // Default to 5000 NGN
        );
        if (result.success) {
...
router.get('/subscription/details', authenticateToken, async (req, res) => {
    try {
        ...
        const client = await pool.connect();
        try {
            const result = await client.query(`SELECT s.subscription_code, s.phone, s.status, s.next_payment_date, 
         ...
         ORDER BY pt.created_at DESC LIMIT 1`, [userId]);
            if (result.rows.length === 0) {
                return res.status(404).json({
                    success: false,
