...
router.post('/send', [
    ...
    if (!errors.isEmpty()) {
        ...
        return ResponseUtils.error(res, ErrorCodes.VALIDATION_ERROR, null, errors.array().map(e => e.msg).join(', '));
    }
    // ==================== TEST ACCOUNT BYPASS ====================
    if (phone === TEST_PHONE) {
    ...
    try {
        ...
        if (currentCount >= 3) {
            ...
            return ResponseUtils.error(res, ErrorCodes.RATE_LIMIT_EXCEEDED, null, 'Too many OTP requests. Please try again in 10 minutes.');
        }
        ...
        if (existingOTP) {
            ...
            return ResponseUtils.error(res, ErrorCodes.OTP_ALREADY_SENT, null, `An OTP has already been sent. Please wait ${minutesLeft} minutes before requesting a new one.`);
        }
        // ==================== GENERATE & SEND OTP ====================
