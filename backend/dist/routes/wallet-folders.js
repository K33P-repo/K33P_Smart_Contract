...
router.get('/', authenticateToken, async (req, res) => {
    try {
        ...
        const totalWallets = user.folders?.reduce((total, folder) => total + (folder.items?.length || 0), 0) || 0;
 ...
 */
router.post('/folders', authenticateToken, folderLimiter, async (req, res) => {
    try {
        const userId = req.user?.userId;
        const { name } = req.body;
        if (!userId) {
 ...
 */
router.post('/folders/:folderId/wallets', authenticateToken, walletLimiter, async (req, res) => {
    try {
        const userId = req.user?.userId;
        const { folderId } = req.params;
        const { name } = req.body; // Only name is required
        if (!userId) {
            return res.status(401).json({ success: false, message: 'User not authenticated' });
        }
        ...
        const updatedFolders = user.folders.map(f => f.id === folderId ? updatedFolder : f);
 ...
 */
router.put('/folders/:folderId/wallets/:walletId', authenticateToken, walletLimiter, async (req, res) => {
    try {
        const userId = req.user?.userId;
        const { folderId, walletId } = req.params;
        const { name, keyType, fileId } = req.body;
        if (!userId) {
            return res.status(401).json({ success: false, message: 'User not authenticated' });
        }
        ...
        if (name) {
            const duplicateWallet = folder.items.find(w => w.id !== walletId && w.name === name.trim());
            if (duplicateWallet) {
                return res.status(409).json({ success: false, message: 'A wallet with this name already exists in this folder' });
            }
        }
 ...
 */
router.patch('/folders/:folderId/wallets/:walletId/recovery', authenticateToken, walletLimiter, async (req, res) => {
    try {
        const userId = req.user?.userId;
        const { folderId, walletId } = req.params;
        const { keyType, fileId } = req.body;
        if (!userId) {
            return res.status(401).json({ success: false, message: 'User not authenticated' });
        }
        if (!keyType || !fileId) {
...
router.delete('/folders/:folderId/wallets/:walletId', authenticateToken, walletLimiter, async (req, res) => {
    try {
        const userId = req.user?.userId;
        const { folderId, walletId } = req.params;
        if (!userId) {
 ...
 */
router.delete('/folders/:folderId', authenticateToken, folderLimiter, async (req, res) => {
    try {
        const userId = req.user?.userId;
        const { folderId } = req.params;
        if (!userId) {
        ...
        if (walletsWithFileIds.length > 0) {
            ...
            const cleanupPromises = walletsWithFileIds.map(wallet => fetch('https://k33p-backend.onrender.com/api/v1/vault/delete', {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ file_id: wallet.fileId }),
            }).catch(error => {
...
//# sourceMappingURL=wallet-folders.js.map