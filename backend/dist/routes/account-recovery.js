...
const backupPhraseAttempts = new Map();
// ============================================================================
...
const hashBackupPhrase = (phrase, salt) => {
    return crypto.createHash('sha256').update(phrase + salt).digest('hex');
};
const generateSalt = () => {
    return crypto.randomBytes(32).toString('hex');
};
const sendEmergencyEmail = async (email, token, userName) => {
    try {
...
const findUserByIdentifier = async (identifier) => {
    try {
        ...
        let result = await client.query('SELECT * FROM users WHERE phone_hash = $1', [phoneHash]);
        if (result.rows.length === 0) {
            // Try to find by email
            result = await client.query('SELECT * FROM users WHERE email = $1', [identifier]);
        }
        client.release();
 ...
 */
router.post('/initiate', validateRecoveryInit, handleValidationErrors, handleAsyncRoute(async (req, res) => {
    try {
        ...
        switch (recoveryMethod) {
            case 'emergency_contact':
                ...
                const client = await pool.connect();
                const emergencyResult = await client.query('SELECT * FROM emergency_contacts WHERE user_id = $1 AND verified = true', [user.user_id]);
                client.release();
 ...
 */
router.post('/verify-backup-phrase', validateBackupPhrase, handleValidationErrors, handleAsyncRoute(async (req, res) => {
    try {
        ...
        const client = await pool.connect();
        const result = await client.query('SELECT phrase_hash, salt FROM backup_phrases WHERE user_id = $1', [recoveryRequest.userId]);
        client.release();
        if (result.rows.length === 0) {
            return res.status(400).json({
                success: false,
 ...
 */
router.post('/verify-emergency-contact', validateEmergencyContact, handleValidationErrors, handleAsyncRoute(async (req, res) => {
    try {
 ...
 */
router.post('/verify-onchain', validateOnchainProof, handleValidationErrors, handleAsyncRoute(async (req, res) => {
    try {
        ...
        const verificationResult = await k33pManager.verifyTransactionByWalletAddress(walletAddress, 2000000n // 2 ADA
        );
 ...
 */
router.get('/status/:recoveryId', param('recoveryId').isUUID().withMessage('Valid recovery ID is required'), handleValidationErrors, handleAsyncRoute(async (req, res) => {
    try {
 ...
 */
async function completeRecovery(recoveryRequest) {
    try {
        ...
        if (recoveryRequest.verificationData.backupPhraseVerified) {
            const client = await pool.connect();
            await client.query('UPDATE backup_phrases SET last_used = CURRENT_TIMESTAMP WHERE user_id = $1', [recoveryRequest.userId]);
            client.release();
        }
    }
    catch (error) {
