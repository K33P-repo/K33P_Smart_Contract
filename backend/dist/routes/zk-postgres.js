...
router.post('/login-with-pin', async (req, res) => {
    try {
        ...
        try {
            ...
            const userQuery = await client.query('SELECT user_id as "userId", wallet_address as "walletAddress", phone_hash as "phoneHash", zk_commitment as "zkCommitment", pin FROM users WHERE phone_hash = $1', [phoneHash]);
            ...
            const proofQuery = await client.query('SELECT proof, commitment, public_inputs, is_valid FROM zk_proofs WHERE user_id = $1 AND is_valid = true ORDER BY created_at DESC LIMIT 1', [user.userId]);
