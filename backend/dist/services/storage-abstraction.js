...
export class StorageAbstractionService {
    ...
    // ============================================================================
    async storeInIagon(key, data) {
        const jsonData = JSON.stringify(data);
        return await storeData(key, jsonData);
    }
    async retrieveFromIagon(key) {
        const jsonData = await retrieveData(key);
        return JSON.parse(jsonData);
    }
    async updateInIagon(key, data) {
        const jsonData = JSON.stringify(data);
        await updateData(key, jsonData);
    }
    async deleteFromIagon(key) {
        await deleteData(key);
    }
    // ============================================================================
    // POSTGRESQL STORAGE OPERATIONS
    // ============================================================================
    async storeInPostgreSQL(tableName, data) {
        const client = await pool.connect();
        try {
            let query;
            let values;
    ...
    async retrieveFromPostgreSQL(tableName, query) {
        const client = await pool.connect();
        try {
            let sqlQuery;
            let values;
    ...
    async updateInPostgreSQL(tableName, id, data) {
        const client = await pool.connect();
        try {
            let query;
            let values;
    ...
    async deleteFromPostgreSQL(tableName, id) {
        const client = await pool.connect();
        try {
    ...
    async storeUserDeposit(depositData) {
        const selectedStorage = this.selectStorage();
        try {
            const result = await this.withRetry(async () => {
                if (selectedStorage === 'iagon') {
...
export const storageService = new StorageAbstractionService({
    primaryStorage: process.env.PRIMARY_STORAGE || 'iagon',
...
//# sourceMappingURL=storage-abstraction.js.map