...
class SubscriptionService {
    ...
    async getSubscriptionStatus(userId) {
        ...
        try {
            const result = await client.query(`SELECT tier, is_active, start_date, end_date, auto_renew 
    ...
    async updateSubscription(userId, updates) {
        ...
        try {
            ...
            if (existingSubscription.rows.length === 0) {
                ...
                await client.query(`INSERT INTO subscriptions (user_id, tier, is_active, start_date, end_date, auto_renew)
           VALUES ($1, $2, $3, $4, $5, $6)`, [
                    userId,
                ...
                if (updateFields.length > 0) {
                    values.push(userId);
                    await client.query(`UPDATE subscriptions SET ${updateFields.join(', ')} WHERE user_id = $${paramIndex}`, values);
                }
            }
     ...
     */
    async initializePremiumSubscription(userId, email, durationMonths = 1, amount = 5000 // 5000 NGN = ~$5 USD
    ) {
        try {
    ...
    async verifyAndActivateSubscription(reference) {
        try {
            ...
            const userId = metadata.userId ||
                (metadata.custom_fields && metadata.custom_fields.find((f) => f.variable_name === 'user_id')?.value);
            const durationMonths = parseInt(metadata.durationMonths ||
                (metadata.custom_fields && metadata.custom_fields.find((f) => f.variable_name === 'duration_months')?.value) ||
                '1');
            if (!userId) {
     ...
     */
    async verifyPaystackWebhookSignature(payload, signature) {
        return await paystackService.verifyWebhookSignature(payload, signature);
    }
    /**
     * Handle successful payment webhook
     */
    async handleSuccessfulPaymentWebhook(data) {
        try {
     ...
     */
    async handleSubscriptionCreatedWebhook(data) {
     ...
     */
    async handleSubscriptionCancelledWebhook(data) {
        try {
            // Find user by email and cancel subscription
            const client = await pool.connect();
            try {
                const userResult = await client.query('SELECT user_id FROM users WHERE email = $1', [data.customer.email]);
    ...
    async getExpiringSubscriptions() {
        const client = await pool.connect();
        try {
            const result = await client.query(`SELECT user_id FROM subscriptions 
         WHERE tier = 'premium' 
         AND is_active = true
     ...
     * Initialize monthly recurring subscription with phone number
     */
    async initializeMonthlyRecurringSubscription(userId, phone, amount = 5000 // 5000 NGN = Monthly subscription
    ) {
        try {
            // Check if user already has active premium subscription
            const currentStatus = await this.getSubscriptionStatus(userId);
     ...
     */
    async handleRecurringPaymentWebhook(event) {
        try {
     ...
     */
    async handleRecurringChargeSuccess(data) {
        try {
            const metadata = data.metadata || {};
            const userId = metadata.userId;
     ...
     */
    async handleRecurringSubscriptionCreated(data) {
        try {
         ...
         WHERE customer_code = $3`, [data.subscription_code, new Date(data.next_payment_date), data.customer.customer_code]);
            }
            finally {
                client.release();
            }
     ...
     */
    async handleMonthlyInvoiceCreated(data) {
     ...
     */
    async cancelRecurringSubscription(userId) {
        try {
            // Get subscription code from database
            const client = await pool.connect();
            try {
                const result = await client.query('SELECT subscription_code FROM subscriptions WHERE user_id = $1', [userId]);
    ...
    async getSubscriptionStatistics() {
        ...
        try {
            const [totalResult, premiumResult, freemiumResult, activeResult, expiredResult, expiringResult] = await Promise.all([
                ...
                client.query("SELECT COUNT(*) FROM subscriptions WHERE tier = 'premium' AND is_active = true AND end_date > CURRENT_TIMESTAMP"),
                client.query("SELECT COUNT(*) FROM subscriptions WHERE tier = 'premium' AND is_active = true AND end_date <= CURRENT_TIMESTAMP"),
