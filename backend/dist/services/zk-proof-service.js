...
export class ZKProofService {
    static async generateAndStoreUserZKProof(userData) {
        try {
            ...
            const proofInputs = {};
            if (userData.phoneNumber)
                proofInputs.phone = userData.phoneNumber;
            if (userData.biometricData)
                proofInputs.biometric = userData.biometricData;
            if (userData.passkeyData)
                proofInputs.passkey = userData.passkeyData;
    ...
    static async generateAndStoreDataZKProof(userId, dataType, data) {
        try {
    ...
    static async storeZKProofInDatabase(zkData) {
        const client = await pool.connect();
        try {
            await client.query('INSERT INTO zk_proofs (user_id, commitment, proof, public_inputs, is_valid, verified_at) VALUES ($1, $2, $3, $4, $5, $6)', [
                zkData.userId,
                zkData.commitment,
    ...
    static async getUserZKProofs(userId) {
        const client = await pool.connect();
        try {
            const result = await client.query('SELECT * FROM zk_proofs WHERE user_id = $1 ORDER BY created_at DESC', [userId]);
    ...
    static async verifyStoredZKProof(userId, commitment) {
        const client = await pool.connect();
        try {
            const result = await client.query('SELECT is_valid FROM zk_proofs WHERE user_id = $1 AND commitment = $2', [userId, commitment]);
    ...
    static async getLatestUserZKProof(userId) {
        const client = await pool.connect();
        try {
            const result = await client.query('SELECT * FROM zk_proofs WHERE user_id = $1 ORDER BY created_at DESC LIMIT 1', [userId]);
            if (result.rows.length === 0) {
                return null;
            }
    ...
    static async getUserByPhoneHash(phoneHash) {
        const client = await pool.connect();
        try {
            ...
            const result = await client.query('SELECT user_id as "userId", wallet_address as "walletAddress", phone_hash as "phoneHash", zk_commitment as "zkCommitment", auth_methods as "authMethods", verification_method as "verificationMethod", verified FROM users WHERE phone_hash = $1', [phoneHash]);
    ...
    static async getUserById(userId) {
        const client = await pool.connect();
        try {
            const result = await client.query('SELECT user_id as "userId", wallet_address as "walletAddress", phone_hash as "phoneHash", zk_commitment as "zkCommitment", auth_methods as "authMethods", verification_method as "verificationMethod", verified FROM users WHERE user_id = $1', [userId]);
            if (result.rows.length === 0) {
                return null;
            }
            const row = result.rows[0];
    ...
    static async getUserByWalletAddress(walletAddress) {
        const client = await pool.connect();
        try {
            const result = await client.query('SELECT user_id as "userId", wallet_address as "walletAddress", phone_hash as "phoneHash", zk_commitment as "zkCommitment", auth_methods as "authMethods", verification_method as "verificationMethod", verified FROM users WHERE wallet_address = $1', [walletAddress]);
            if (result.rows.length === 0) {
                return null;
            }
            const row = result.rows[0];
    ...
    static async updateUser(userId, updates) {
        ...
        try {
            ...
            const result = await client.query(`UPDATE users SET ${setClause} WHERE user_id = $1 RETURNING user_id as "userId", wallet_address as "walletAddress", phone_hash as "phoneHash", zk_commitment as "zkCommitment", auth_methods as "authMethods", verification_method as "verificationMethod", verified`, [userId, ...values]);
            return result.rows[0];
        }
        finally {
            client.release();
...
//# sourceMappingURL=zk-proof-service.js.map