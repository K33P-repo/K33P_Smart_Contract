...
import { configureNokProviders, joinNok, registerNok, approveNokLogin, isNokRegistered, getNokLedgerState, } from '@k33p/nok-cli/dist/nok-api.js';
...
const nokLogger = {
    info: (msg, ...rest) => logger.info(`[NOK] ${String(msg)}`, ...rest),
    warn: (msg, ...rest) => logger.warn(`[NOK] ${String(msg)}`, ...rest),
    error: (msg, ...rest) => logger.error(`[NOK] ${String(msg)}`, ...rest),
    debug: (msg, ...rest) => logger.debug?.(`[NOK] ${String(msg)}`, ...rest),
    trace: () => { },
    fatal: (msg, ...rest) => logger.error(`[NOK] ${String(msg)}`, ...rest),
    child: () => nokLogger,
};
function requireEnv(name) {
    const v = process.env[name]?.trim();
    if (!v)
        throw new Error(`NOK: missing required env var ${name}`);
    return v;
}
...
export async function registerNokForUser(userId, nokIdentifier) {
...
export async function approveNokLoginForUser(userId, nokIdentifier) {
    const { deployed } = await getContext();
    const ownerField = ownerIdentifierToField(userId);
    const nokField = nokHashToField(nokIdentifier);
...
export async function checkNokRegisteredForUser(userId) {
...
//# sourceMappingURL=nok-service.js.map