...
interface PhoneChangeRequest {
  ...
  verificationMethod: 'onchain' | 'offchain';
  otp?: string;
  zkProof?: string;
  txHash?: string;
  timestamp: Date;
...
const verifyPin = async (userId: string, pin: string): Promise<boolean> => {
  try {
    ...
    const result = await client.query(
      'SELECT auth_hash FROM auth_data WHERE user_id = $1 AND auth_type = $2 AND is_active = true',
      [userId, 'pin']
    );
    client.release();
...
const sendRecoveryEmail = async (email: string, recoveryData: any): Promise<boolean> => {
  ...
  handleAsyncRoute(async (req: Request, res: Response) => {
    try {
      ...
      const existingUser = await client.query(
        'SELECT user_id FROM users WHERE phone_hash = $1 AND user_id != $2',
        [newPhoneHash, userId]
      );
      client.release();
 ...
 */
router.post('/change/verify-onchain',
  authenticateToken,
  body('requestId').isUUID().withMessage('Valid request ID is required'),
  body('txHash').isLength({ min: 64, max: 64 }).withMessage('Valid transaction hash is required'),
  handleValidationErrors,
  handleAsyncRoute(async (req: Request, res: Response) => {
    try {
