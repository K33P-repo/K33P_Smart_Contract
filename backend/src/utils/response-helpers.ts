 ...
 */
export const handleValidationErrorsMiddleware = (req: any, res: Response, next: any) => {
...
export function handleValidationErrors(errors: any[]): never {
...
export function createErrorResponse(code: ErrorCodes, message?: string, details?: string) {
  throw new K33PError(code, message, details);
}

// Specific scenario handlers
export function handleUserAlreadyExists(details?: string): never {
  throw new K33PError(ErrorCodes.USER_ALREADY_EXISTS, undefined, details);
}

export function handlePhoneAlreadyExists(details?: string): never {
  throw new K33PError(ErrorCodes.PHONE_ALREADY_EXISTS, undefined, details);
}

export function handleWalletAlreadyExists(details?: string): never {
  throw new K33PError(ErrorCodes.WALLET_ALREADY_EXISTS, undefined, details);
}

export function handleRefundFailed(details?: string): never {
...
export const checkUserExists = async (
  userLookupFn: Function,
  identifier: string,
  identifierType: 'phone' | 'wallet' | 'userId' = 'userId'
) => {
  try {
...
export const checkUserStatus = async (
  user: any,
  checkRefundStatus: boolean = true
) => {
 ...
 */
export const handleRefundError = (error: any, user?: any) => {
  if (error.message?.includes('insufficient funds')) {
    throw new K33PError(
      ErrorCodes.REFUND_FAILED,
      error,
      'Insufficient funds in refund wallet. Our team has been notified.'
    );
  }
  ...
  throw new K33PError(
    ErrorCodes.REFUND_FAILED,
    error,
    'Refund processing failed. Our team has been notified and will resolve this issue.'
  );
};
...
export const handleTransactionError = (error: any) => {
  if (error.message?.includes('not found')) {
    throw new K33PError(
      ErrorCodes.DEPOSIT_VERIFICATION_FAILED,
      error,
      'Transaction not found. Please ensure the transaction is confirmed on the blockchain.'
    );
  }
  ...
  if (error.message?.includes('insufficient amount')) {
    throw new K33PError(
      ErrorCodes.DEPOSIT_VERIFICATION_FAILED,
      error,
      'Insufficient deposit amount. Please send the required 2 ADA.'
    );
  }
  ...
  throw new K33PError(
    ErrorCodes.DEPOSIT_VERIFICATION_FAILED,
    error,
    'Could not verify your ADA deposit. Please ensure the transaction is confirmed.'
  );
};
 ...
 */
export const handleRateLimitError = (req: any, res: Response) => {
 ...
 */
export const handleExternalServiceError = (serviceName: string, error: any) => {
